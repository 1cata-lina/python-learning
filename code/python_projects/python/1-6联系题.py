# Python
# 语感训练涵盖列表、字典、元组、集合、字符串、格式化输出、类型和整数转换、文件读写、杂项等9个类别，共100道练习题。
...
# 11.将列表[3, 0, 8, 5, 7]中大于5元素置为1，其余元素置为0。
...
# 17.若a = [1, 2, 3]，令b = a，执行b[0] = 9， a[0]亦被改变。为何？如何避免？
...
# 22.向字典{‘Alice’: 20, ‘Beth’: 18, ‘Cecil’: 21} 中追加 ‘David’:19键值对，更新Cecil的值为17。


# 23.删除字典{‘Alice’: 20, ‘Beth’: 18, ‘Cecil’: 21} 中的Beth键后，清空该字典。

# 24.判断David和Alice是否在字典{‘Alice’: 20, ‘Beth’: 18, ‘Cecil’: 21} 中。

# 25.遍历字典{‘Alice’: 20, ‘Beth’: 18, ‘Cecil’: 21}，打印键值对。

# 26.若a = dict()，令b = a，执行b.update({‘x’:1})， a亦被改变。为何？如何避免？
...
# 34.返回在元组(2, 5, 3, 7)索引号为2的位置插入元素9之后的新元组。
...
# 55.清除字符串 ‘\tpython \n’ 左侧、右侧，以及左右两侧的空白字符。
# 56.将三个全英文字符串（比如，‘ok’, ‘hello’, ‘thankyou’）分行打印，实现左对齐、右对齐和居中对齐效果。
...
# 62.从键盘输入手机号码，输出形如 ‘Mobile: 18666777788’ 的字符串。
# 63.从键盘输入年月日时分秒，输出形如 ‘2019 - 05 - 0112: 00:00’ 的字符串。
# 64.给定两个浮点数3.1415926和2.7182818，格式化输出字符串 ‘pi = 3.1416, e = 2.7183’。
# 65.将0.00774592和356800000格式化输出为科学计数法字符串。
# 66.将十进制整数240格式化为八进制和十六进制的字符串。
# 67.将十进制整数240转为二进制、八进制、十六进制的字符串。
...
# 72.返回ASCII编码值为57和122的字符。
# 73.将二维列表[[0.468, 0.975, 0.446], [0.718, 0.826, 0.359]]写成名为csv_data的csv格式的文件，并尝试用excel打开它。
# 74.从csv_data.csv文件中读出二维列表。
# 75.向csv_data.csv文件追加二维列表[[1.468, 1.975, 1.446], [1.718, 1.826, 1.359]]，然后读出所有数据。
# 76.交换变量x和y的值。
# 77.判断给定的参数x是否是整形。
# 78.判断给定的参数x是否为列表或元组。
...
# 81.将列表[3,‘a’, 5.2, 4, {}, 9, []] 中大于3的整数或浮点数置为1，其余置为0。
# 82.a, b是两个数字，返回其中较小者或最大者。
# 83.找到列表[8, 5, 2, 4, 3, 6, 5, 5, 1, 4, 5]中出现最频繁的数字以及出现的次数。
# 84.将二维列表[[1], [‘a’, ‘b’], [2.3, 4.5, 6.7]] 转为一维列表。
# 85.将等长的键列表和值列表转为字典。
...
# 96.使用map函数求列表[2, 3, 4, 5]中每个元素的立方根。
# 97.使用sys.stdin.readline()写一个和input()函数功能完全相同的函数。
# 98.使用二维列表描述9x9围棋局面，'w’表示白色棋子，‘b’表示黑色棋子，’-'表示无子，打印成下图左所示的文本棋盘。
# 99.对于9x9围棋盘，用a - i标识各行，用1 - 9标识各列，设计函数go()，输入位置和颜色，即输出文本棋盘，模拟围棋对弈的过程。
# 100.下图中是国际跳棋的初始局面，10x10的棋盘上只有50个深色格子可以落子，'w’表示白色棋子，‘b’表示黑色棋子，’-'表示无子，字符串phase = ‘b’*20 + ‘-’*10 + ‘w’*20
# 表示下图中的局面，请将phase打印成下图右所示的样子。在这里插入图片描述
# # # 去除字符串s='asas352saf3r3#%#@crv@#$13xd'得符号
